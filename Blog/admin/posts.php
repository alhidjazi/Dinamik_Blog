          ...
          <ol class="breadcrumb">
            ...
            <?php
                if(isset($_GET["delete"])){
                    ...
                    //...
                    $query2 = "SELECT * FROM posts WHERE post_id = {$del_post_id}";
                    $select_del_image = mysqli_query($conn, $query2);
                    if(!$select_del_image){
                        die("Bağlantı hatalıdır!: ". mysqli_errno($conn));
                    }
                    while($row = mysqli_fetch_assoc($select_del_image)){
                        $post_image = $row["post_image"];
